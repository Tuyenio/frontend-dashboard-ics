  ...
  return (
    ...
    >
      ...
      <div className="relative group">
        ...
        <div className="relative bg-slate-900 dark:bg-slate-950 light:bg-white rounded-2xl overflow-hidden shadow-2xl border border-slate-800 dark:border-slate-700 light:border-slate-200">
          {/* Video Wrapper with 16:9 aspect ratio */}
          <div className="relative aspect-video bg-slate-950 dark:bg-black light:bg-slate-100">
          ...
          {(title || subtitle) && (
            <motion.div
              ...
              viewport={{ once: true }}
              className="p-6 bg-gradient-to-r from-slate-900/95 to-slate-800/95 dark:from-slate-900/95 dark:to-slate-800/95 light:from-slate-50 light:to-white backdrop-blur-sm"
            >
              {title && (
                <h3 className="text-xl font-bold text-white dark:text-white light:text-slate-900 mb-2">{title}</h3>
              )}
              {subtitle && (
                <p className="text-slate-400 dark:text-slate-400 light:text-slate-600 text-sm">{subtitle}</p>
              )}
            </motion.div>
          )}
        </div>
